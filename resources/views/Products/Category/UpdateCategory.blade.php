...
<body>
    ...
    <form class="form" action="{{ route('category.update',$Category->CategoryId) }}" method="POST" enctype="multipart/form-data">
        ...
        <img src="{{ asset('storage/' . $Category->CategoryImage) }}" id="Avatar" >
        ...
        @else
            ...
            <select  class="MainInput" id="Brands" name="BrandId">
                ...
                @foreach ($Brands as $Brand )
                    <option value="{{ $Brand->BrandId }}" {{ $Brand->BrandId == $Category->BrandId ? 'selected' : ''}}> {{ $Brand->BrandName }} </option>
                @endforeach
        ...
        <input type="text" class="MainInput" placeholder="Category Name" name="CategoryName" value="{{$Category->CategoryName}}">

        <input type="submit" value="submit" class="SubmitForm">
        <input type="text" id="path" name="path" hidden value="{{$Category->CategoryImage}}">
